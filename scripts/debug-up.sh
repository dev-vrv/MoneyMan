...
COMPOSE_FILE="$ROOT_DIR/docker-compose.debug.yml"
...
docker compose -f "$COMPOSE_FILE" run --rm backend python manage.py shell -c "from django.contrib.auth import get_user_model; User = get_user_model(); email = \"$SUPERUSER_EMAIL\"; password = \"$SUPERUSER_PASSWORD\"; user, _ = User.objects.get_or_create(username=email, defaults={'email': email}); user.email = email; user.is_staff = True; user.is_superuser = True; user.set_password(password); user.save(); print(f'Superuser ensured: {email}')"
